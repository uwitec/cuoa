...
<script src="<%=contextPath %>/js/class.js" type="text/javascript"></script>
 <script type="text/javascript">
 	   	...
 	   	//初始化数据
	   	$(function(){
	   		fetch_return_class_value("<s:property value='reject.courseId'/>");
	   		
	   		$("#remark").val("<s:property value='reject.remark'/>");
	   		$("#rejectCourseType").val(<s:property value='reject.rejectCourseType'/>);
	   		$("#termOfRejectCourseId").val("<s:property value='reject.termOfRejectCourseId'/>");
	   		
	   		var isHeadTeacher = "<s:property value='reject.isHeadTeacher'/>";
   			$("input:radio[name='reject.isHeadTeacher'][value="+isHeadTeacher+"]").attr("checked","checked");
   
			var verifyYearMonth = "<s:property value='reject.verifyYearMonth'/>";
			if (verifyYearMonth.length == 7) {
				$("#verify_year").val(verifyYearMonth.split("-")[0]);
				$("#verify_month").val(verifyYearMonth.split("-")[1]);
			}
			setTimeout(function() {
   			$("#lessonNo").val(<s:property value='reject.lessonNo'/>);
   			lessonNoOnchange();
   		},2000); 
	   	});
	     ...
	     function verify()
	     {
			...
			if(reasonForModify.length>200)  {  alert("修改原因,应在200字符以内!","warn");	return false;}  
    	...
    	//提交
         function f_submit(){
        	...
        	if($("form.validate").valid()){
	        	...
	        	$.post(contextPath + "/py-rejectcourse/rejectcourse!edit.action", param, function(json){
					if(json.success){
...
<body>
 <form class="l-form validate">
    <div class="content">
    	<ul>
    		<li>
    			<input type="hidden" name="reject.id" id="rejectId" value="<s:property value='reject.id'/>">
    			<input type="hidden" name="reject.courseId" id="courseId" value="<s:property value='reject.courseId'/>">
    			<input type="hidden" name="reject.verifyYearMonth" id="verifyYearMonth" value="<s:property value='reject.verifyYearMonth'/>">
				<input type="hidden" name="reject.rejectingTeacherId" id="rejectingTeacherId" value="<s:property value='reject.rejectingTeacherId'/>">
				<input type="hidden" name="reject.receivingTeacherId" id="receivingTeacherId" value="<s:property value='reject.receivingTeacherId'/>">
	   			<label>确认年月:</label>
    	...
    	<ul>    	
    		<li>
    			...
    			<input name ="reject.isHeadTeacher" type="radio" value=0 onchange="teacher_isHeadteacher_onchange()" > 老师
    	...
    	<ul>
    		<li>
    			...
    			<input type="text" id="rejectingTeacherName" class="input-text" readonly="readonly" value="<s:property value='reject.rejectingTeacherName'/>"/>
    		...
    		<li>
    			...
    			<input id="rejectDate" name="reject.rejectDate" type="text" class="Wdate input-text"  value="<fmt:formatDate value='${reject.rejectDate}' pattern='yyyy-MM-dd'/>" 
    		...
    		<li>
				...
				<select id="rejectCourseType" name="reject.rejectCourseType" class="input-select">
					<option value="">请选择</option>
    	...
    	<ul>
    		<li>
    			...
    			<input type="text"  id="receivingTeacherName" class="input-text" readonly="readonly" value="<s:property value='reject.receivingTeacherName'/>"/>
    	...
    	<ul>
    		<li>
    			<textarea  rows="11" cols="100" readonly="readonly"><s:property value='reject.reason'/></textarea>		
    		</li>
    	</ul>    	
    	<ul>
    		<li>
    	...
    	<ul>
    		<li>
   			<textarea name="reject.reasonForModify"  id="reasonForModify" rows="11" cols="100"></textarea>		
    		</li>
    	</ul>    	
    </div>
    </form>
