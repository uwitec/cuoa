...
import com.oa.framework.utils.file.newppt.FtpClientUtil;
import com.oa.framework.utils.string.MD5;
public class TraversingPPT2FilesByFTP {
	...
	public static void main(String[] args) throws IOException {
    	//TraversingPPT2FilesByFTP ftp = new TraversingPPT2FilesByFTP();
    	//ftp.getFileNameAndPathList();
		getFileNameAndPathList("010");
    }

	public static FtpClientUtil connection(String areaCode) throws IOException {
		FtpClientUtil ftpClientUtil = new FtpClientUtil();
		System.out.println(FileUtils.getFileParam("ppt2_ftp_server_" + areaCode).toString());
    	System.out.println(FileUtils.getFileParam("ppt2_ftp_user_" + areaCode).toString());
    	System.out.println(FileUtils.getFileParam("ppt2_ftp_pwd_" + areaCode).toString());
    	System.out.println(FileUtils.getFileParam("ppt2_ftp_rootpath_" + areaCode).toString());
    	ftpClientUtil.connectionServer(FileUtils.getFileParam("ppt2_ftp_server_" + areaCode).toString(), FileUtils.getFileParam("ppt2_ftp_user_" + areaCode).toString(), 
    			FileUtils.getFileParam("ppt2_ftp_pwd_" + areaCode).toString(), FileUtils.getFileParam("ppt2_ftp_rootpath_" + areaCode).toString());
	...
	public static List<Map<String, String>> getFileNameAndPathList(String areaCode) throws IOException {
		...
		List<String> in_files = ftpClientUtil.getFileList("ppt2_ftp_encoding_" + areaCode);
		...
		for (String in_file: in_files) {
			...
			sourceFileNameAndPathMap.put("filePath", 
					FileUtils.getFileParam("ppt2_ftp_rootpath_" + areaCode).toString() + "/" + sourceFileNameAndPathMap.get("fileName")); //MD5 path
			sourceFileNameAndPathList.add(sourceFileNameAndPathMap);
		}
		
		//遍历目标文件夹
		ftpClientUtil.cdPath(FileUtils.getFileParam("ppt2_ftp_newfolder_" + areaCode).toString());
		in_files = ftpClientUtil.getFileList("ppt2_ftp_encoding_" + areaCode);
		...
		for (String in_file: in_files) {
			...
			targetFileNameAndPathMap.put("filePath", FileUtils.getFileParam("ppt2_ftp_newfolder_" + areaCode).toString() + "/" + in_file);
